...
// .wrangler/tmp/bundle-KHucEP/strip-cf-connecting-ip-header.js
...
function calculateChecksum(base32Str) {
  let checksum = 0;
...
function generateAllianceGlyph() {
...
__name(hashGlyph, "hashGlyph");
function generateAuthToken() {
  return crypto.randomUUID();
}
__name(generateAuthToken, "generateAuthToken");

// src/universeSeeder.ts
function cleanUniverseId(raw) {
  if (!raw)
    return "s267-en";
...
__name(cleanUniverseId, "cleanUniverseId");
function normalizePlayerStatus(raw) {
  if (!raw || raw === "active")
    return "active";
  const clean = raw.trim();
  const tags = /* @__PURE__ */ new Set();
  if (clean.includes(",")) {
    clean.split(",").forEach((t2) => {
      const part = t2.trim();
...
__name(normalizePlayerStatus, "normalizePlayerStatus");
async function isUniverseSeeded(rawUniverseId, env2) {
...
__name(isUniverseSeeded, "isUniverseSeeded");
async function seedUniverseFromOfficialAPI(rawUniverseId, env2) {
  ...
  const serverData = serverDataRes.ok ? await serverDataRes.json() : {};
  const alliancesData = alliancesRes.ok ? await alliancesRes.json() : {};
  ...
  const alliancesMap = /* @__PURE__ */ new Map();
  ...
  for (const item of planetList) {
    ...
    if (isNaN(galaxy) || isNaN(system) || isNaN(slot))
      continue;
    let hasMoon = false;
    let moonId;
    let moonName;
    ...
    if (item?.moon) {
      ...
      hasMoon = true;
      moonsCount++;
      moonId = moonAttr?.id ? String(moonAttr.id) : void 0;
      moonName = moonAttr?.name ? String(moonAttr.name) : "Moon";
      moonSize = moonAttr?.size ? parseInt(String(moonAttr.size), 10) : void 0;
    }
    parsedPlanets.push({
      id: String(attr.id || ""),
  ...
  if (!isInitialSeed && existingRows.results) {
    existingRows.results.forEach((r2) => {
      existingMap.set(`${r2.galaxy}:${r2.system}:${r2.slot}`, r2);
    });
  }
  const now = Date.now();
  ...
  for (const p2 of parsedPlanets) {
    const key = `${p2.galaxy}:${p2.system}:${p2.slot}`;
    incomingKeys.add(key);
    const player = playersMap.get(p2.playerId);
    ...
    if (!isInitialSeed && oldSlot) {
      if (oldSlot.player_id !== p2.playerId) {
        hasChanged = true;
        if (!oldSlot.player_id && p2.playerId) {
          detectedEvents.push({
            eventType: "colonized",
            ...
            playerName,
            oldState: null,
            newState: { playerId: p2.playerId, playerName, planetName: p2.name }
          });
        } else if (oldSlot.player_id && !p2.playerId) {
          detectedEvents.push({
            eventType: "abandoned",
            galaxy: p2.galaxy,
            system: p2.system,
      ...
      if (oldSlot.player_id === p2.playerId && oldSlot.player_name && playerName && oldSlot.player_name.trim() !== playerName.trim() && !/^\d+$/.test(playerName.trim())) {
        hasChanged = true;
        detectedEvents.push({
      ...
      if (oldSlot.player_id && p2.playerId && oldStatusNorm !== newStatusNorm) {
        hasChanged = true;
        detectedEvents.push({
      ...
      if ((!oldSlot.has_moon || oldSlot.has_moon === 0) && p2.hasMoon) {
        hasChanged = true;
        detectedEvents.push({
          ...
          playerName,
          oldState: { hasMoon: 0 },
          newState: { hasMoon: 1, moonSize: p2.moonSize }
        });
      } else if (oldSlot.has_moon === 1 && !p2.hasMoon && oldSlot.player_id) {
        hasChanged = true;
        detectedEvents.push({
      ...
      if (oldSlot.planet_name && p2.name && oldSlot.planet_name.trim() !== p2.name.trim()) {
        hasChanged = true;
      }
    ...
    const slotLastChangedAt = hasChanged ? now : oldSlot ? oldSlot.last_changed_at || null : null;
    statements.push(
      env2.DB.prepare(insertQuery).bind(
        universeId,
        p2.galaxy,
        p2.system,
        p2.slot,
  ...
  if (!isInitialSeed) {
    for (const [key, oldSlot] of existingMap.entries()) {
      if (!incomingKeys.has(key) && oldSlot.player_id) {
        const [g2, s2, p2] = key.split(":").map(Number);
        detectedEvents.push({
          eventType: "abandoned",
          playerId: oldSlot.player_id,
          playerName: oldSlot.player_name,
        ...
        statements.push(
          ...
          `).bind(now, now, universeId, g2, s2, p2)
        );
      }
    }
  }
  for (const evt of detectedEvents) {
    const g2 = evt.galaxy !== void 0 ? evt.galaxy : evt.newState?.galaxy || 1;
    const s2 = evt.system !== void 0 ? evt.system : evt.newState?.system || 1;
    const sl = evt.slot !== void 0 ? evt.slot : evt.newState?.slot || 1;
    statements.push(
      env2.DB.prepare(
...
__name(seedUniverseFromOfficialAPI, "seedUniverseFromOfficialAPI");

// src/galaxySync.ts
var GalaxySlotSchema = external_exports.object({
  slot: external_exports.number().int().min(1).max(16),
  planetName: external_exports.string().nullable().optional(),
  planetImage: external_exports.string().nullable().optional(),
  playerId: external_exports.string().nullable().optional(),
  playerName: external_exports.string().nullable().optional(),
  playerStatus: external_exports.string().nullable().optional(),
  // 'active', 'i', 'I', 'v', 'b', 'o'
  playerRank: external_exports.number().nullable().optional(),
  allianceTag: external_exports.string().nullable().optional(),
  hasMoon: external_exports.number().int().min(0).max(1).default(0),
  moonSize: external_exports.number().nullable().optional(),
  debrisMetal: external_exports.number().default(0),
  debrisCrystal: external_exports.number().default(0),
  debrisReaperMetal: external_exports.number().default(0),
  debrisReaperCrystal: external_exports.number().default(0),
  activityMarker: external_exports.string().nullable().optional(),
  // '*', '15m', '35m', etc.
  activityTimestamp: external_exports.number().nullable().optional()
});
var GalaxySyncPayloadSchema = external_exports.object({
  universeId: external_exports.string().min(3).max(30),
  galaxy: external_exports.number().int().min(1).max(9),
  system: external_exports.number().int().min(1).max(499),
  scannedAt: external_exports.number(),
  slots: external_exports.array(GalaxySlotSchema).min(1).max(16)
});
async function authenticateMember(req, env2) {
  ...
  if (!rawToken)
    return null;
  const tokenHash = await hashGlyph(rawToken, env2.SERVER_PEPPER || "nexus_pepper_default");
...
__name(authenticateMember, "authenticateMember");
async function handleGalaxySync(req, env2) {
  try {
    const member = await authenticateMember(req, env2);
    if (!member) {
      return r({ error: "Unauthorized: Invalid or missing token." }, { status: 401 });
    }
    if ("error" in member && member.error === "subscription_expired") {
      return r({ error: "Alliance subscription has expired." }, { status: 403 });
    }
    const body = await req.json();
    const parseResult = GalaxySyncPayloadSchema.safeParse(body);
    if (!parseResult.success) {
      return r({ error: "Invalid galaxy payload", details: parseResult.error.format() }, { status: 400 });
    }
    ...
    for (const newSlot of slots) {
      ...
      if (oldSlot) {
        ...
        } else if (oldSlot.player_id && newSlot.playerId && oldSlot.player_id === newSlot.playerId && oldSlot.player_name && newSlot.playerName && oldSlot.player_name.trim() !== newSlot.playerName.trim() && !/^\d+$/.test(newSlot.playerName.trim()) && // Ignore numbers which are highscore ranks
        ...
        if (oldSlot.player_id && newSlot.playerId && oldStatusNorm !== newStatusNorm) {
          hasSlotChanged = true;
          detectedEvents.push({
            universeId,
            galaxy,
      ...
      const slotLastChangedAt = hasSlotChanged ? now : oldSlot ? oldSlot.last_changed_at || null : null;
      statements.push(
        env2.DB.prepare(
      ...
      if (newSlot.playerId && (newSlot.activityMarker || newSlot.activityTimestamp)) {
        ...
        const hourOfDay = actDate.getUTCHours();
        statements.push(
          env2.DB.prepare(
    ...
    const activeLocks = await env2.DB.prepare(
    ...
    ).bind(universeId, member.alliance_id, `${galaxy}:${system}:%`, now).all();
    return r({
      success: true,
    ...
    return r({ error: "Failed to process galaxy sync", details: err.message }, { status: 500 });
  }
}
__name(handleGalaxySync, "handleGalaxySync");
async function handleGetPlayerActivity(req, env2) {
  try {
    const member = await authenticateMember(req, env2);
    if (!member) {
      return r({ error: "Unauthorized" }, { status: 401 });
    }
    ...
    if (!playerId) {
      return r({ error: "Missing playerId parameter" }, { status: 400 });
    }
    const heatmapRows = await env2.DB.prepare(
      `SELECT day_of_week, hour_of_day, activity_score, last_seen_at
       FROM player_activity_heatmap
       WHERE universe_id = ? AND player_id = ?`
    ).bind(universeId, playerId).all();
    const eventsRows = await env2.DB.prepare(
    ...
    return r({ error: "Failed to get player activity", details: err.message }, { status: 500 });
  }
}
__name(handleGetPlayerActivity, "handleGetPlayerActivity");
async function handleGetGalaxyEvents(req, env2) {
  try {
    const member = await authenticateMember(req, env2);
    const url = new URL(req.url);
    ...
    return r({ error: "Failed to fetch galaxy events", details: err.message }, { status: 500 });
  }
}
__name(handleGetGalaxyEvents, "handleGetGalaxyEvents");
async function handleGetSystem(req, env2) {
  try {
    const member = await authenticateMember(req, env2);
    const url = new URL(req.url);
    const universeId = cleanUniverseId(url.searchParams.get("universeId") || (member && !("error" in member) ? member.universe_id : "s267-en"));
    ...
    if (isNaN(galaxy) || isNaN(system) || galaxy < 1 || galaxy > 9 || system < 1 || system > 499) {
      return r({ error: "Invalid coordinates. Galaxy must be 1-9, system 1-499" }, { status: 400 });
    }
    const slotsRows = await env2.DB.prepare(
    ...
    return r({ error: "Failed to get system data", details: err.message }, { status: 500 });
  }
}
__name(handleGetSystem, "handleGetSystem");

// src/spyReports.ts
var ShareSpyReportSchema = external_exports.object({
  reportId: external_exports.string().min(1).max(50),
  coords: external_exports.string().regex(/^\d{1,2}:\d{1,3}:\d{1,2}$/),
  // e.g. "1:234:5"
  isMoon: external_exports.number().int().min(0).max(1).default(0),
  targetPlayerId: external_exports.string().nullable().optional(),
  targetPlayerName: external_exports.string().nullable().optional(),
  resourcesMetal: external_exports.number().default(0),
  resourcesCrystal: external_exports.number().default(0),
  resourcesDeuterium: external_exports.number().default(0),
  estimatedLoot: external_exports.number().default(0),
  fleetData: external_exports.any().optional(),
  // Structured ships array / object
  defenseData: external_exports.any().optional(),
  // Structured defenses array / object
  buildingsData: external_exports.any().optional(),
  // Structured buildings array / object
  techData: external_exports.any().optional(),
  // Structured techs array / object
  spiedBy: external_exports.string().min(1).max(50).optional(),
  reportTimestamp: external_exports.number()
  // In-game report timestamp in ms
});
async function handleShareSpyReport(req, env2) {
  try {
    const member = await authenticateMember(req, env2);
    if (!member) {
      return r({ error: "Unauthorized: Invalid or missing token." }, { status: 401 });
    ...
    if (!parseResult.success) {
      return r({ error: "Invalid spy report payload", details: parseResult.error.format() }, { status: 400 });
    }
    const report2 = parseResult.data;
    const now = Date.now();
    const spiedByName = report2.spiedBy || member.player_name || "Alliance Scout";
    let loot = report2.estimatedLoot;
    if (!loot && (report2.resourcesMetal || report2.resourcesCrystal || report2.resourcesDeuterium)) {
      loot = Math.floor((report2.resourcesMetal + report2.resourcesCrystal + report2.resourcesDeuterium) * 0.5);
    }
    ...
    ).bind(member.alliance_id, member.universe_id, report2.coords, report2.isMoon).first();
    if (existing && existing.report_timestamp > report2.reportTimestamp) {
      return r({
        success: true,
    ...
    await env2.DB.prepare(
      ...
      report2.coords,
      report2.isMoon,
      report2.targetPlayerId || null,
      report2.targetPlayerName || null,
      report2.resourcesMetal,
      report2.resourcesCrystal,
      report2.resourcesDeuterium,
      loot,
      report2.fleetData ? JSON.stringify(report2.fleetData) : null,
      report2.defenseData ? JSON.stringify(report2.defenseData) : null,
      report2.buildingsData ? JSON.stringify(report2.buildingsData) : null,
      report2.techData ? JSON.stringify(report2.techData) : null,
      spiedByName,
      report2.reportTimestamp,
      now
    ).run();
    return r({
      success: true,
      reportId: report2.reportId,
      coords: report2.coords,
      isMoon: report2.isMoon === 1,
      spiedBy: spiedByName,
      estimatedLoot: loot,
      message: `Spy report for [${report2.coords}] shared with ${member.alliance_name}!`
    });
  } catch (err) {
    console.error("Error in handleShareSpyReport:", err);
    return r({ error: "Failed to share spy report", details: err.message }, { status: 500 });
  }
}
__name(handleShareSpyReport, "handleShareSpyReport");
async function handleGetSpyReports(req, env2) {
  try {
    const member = await authenticateMember(req, env2);
    if (!member) {
      return r({ error: "Unauthorized" }, { status: 401 });
    ...
    `;
    const params = [member.alliance_id, member.universe_id];
    if (coords) {
      query += ` AND coords = ?`;
      params.push(coords);
    }
    ...
    const parsedReports = (rows?.results || []).map((r2) => ({
      ...r2,
      fleetData: r2.fleet_data_json ? JSON.parse(r2.fleet_data_json) : null,
      defenseData: r2.defense_data_json ? JSON.parse(r2.defense_data_json) : null,
      buildingsData: r2.buildings_data_json ? JSON.parse(r2.buildings_data_json) : null,
      techData: r2.tech_data_json ? JSON.parse(r2.tech_data_json) : null
    }));
    return r({
      success: true,
    ...
    return r({ error: "Failed to fetch spy reports", details: err.message }, { status: 500 });
  }
}
__name(handleGetSpyReports, "handleGetSpyReports");
...
var CreateAllianceSchema = external_exports.object({
  ogameAllianceId: external_exports.string().min(1).max(30),
  // In-game alliance ID e.g. "500078"
  allianceDisplayName: external_exports.string().min(2).max(50),
  // Custom display name in Overwatch
  allianceTag: external_exports.string().min(1).max(10),
  // Official in-game tag e.g. "LoW"
  universeId: external_exports.string().min(3).max(60),
  // e.g. "s267-en"
  adminPlayerId: external_exports.string().min(1).max(30),
  adminPlayerName: external_exports.string().min(1).max(50)
});
router.post("/api/v1/alliances/create", async (req, env2) => {
  try {
    const body = await req.json();
    const parsed = CreateAllianceSchema.safeParse(body);
    if (!parsed.success) {
      return r({ error: "Invalid input", details: parsed.error.format() }, { status: 400 });
    }
    const { ogameAllianceId, allianceDisplayName, allianceTag, universeId: rawUni, adminPlayerId, adminPlayerName } = parsed.data;
    const universeId = cleanUniverseId(rawUni);
    const existingPlayerAlly = await env2.DB.prepare(
    ...
    const glyphHash = await hashGlyph(glyphCode, env2.SERVER_PEPPER || "nexus_pepper_default");
    ...
    const adminTokenHash = await hashGlyph(adminToken, env2.SERVER_PEPPER || "nexus_pepper_default");
    await env2.DB.prepare(
    ...
    let seeded = false;
    try {
      const alreadySeeded = await isUniverseSeeded(universeId, env2);
      if (!alreadySeeded) {
        console.log(`[Alliances] Universe ${universeId} not seeded. Fetching official Gameforge XML baseline...`);
        await seedUniverseFromOfficialAPI(universeId, env2);
        seeded = true;
      }
    } catch (seedErr) {
    ...
    return r({
      ...
      allianceId,
      glyphCode,
      // Return the raw Glyph to the Admin to share
    ...
    return r({ error: "Failed to create alliance", details: err.message }, { status: 500 });
  }
});
var JoinAllianceSchema = external_exports.object({
  glyphCode: external_exports.string().min(10).max(30),
  playerId: external_exports.string().min(1).max(30),
  playerName: external_exports.string().min(1).max(50),
  permissions: external_exports.record(external_exports.boolean()).optional(),
  clientVersion: external_exports.string().optional()
});
router.post("/api/v1/auth/join", async (req, env2) => {
  try {
    const body = await req.json();
    ...
    if (!validateAllianceGlyph(glyphCode)) {
      return r({ error: "Invalid or malformed Alliance Glyph code." }, { status: 400 });
    }
    const glyphHash = await hashGlyph(glyphCode, env2.SERVER_PEPPER || "nexus_pepper_default");
    const alliance = await env2.DB.prepare(
    ...
    ).bind(glyphHash).first();
    if (!alliance) {
      return r({ error: "Alliance Glyph not found or has been revoked." }, { status: 404 });
    }
    const now = Date.now();
    ...
    const tokenHash = await hashGlyph(authToken, env2.SERVER_PEPPER || "nexus_pepper_default");
    await env2.DB.prepare(
      `INSERT INTO alliance_members (
        alliance_id, player_id, player_name, auth_token_hash, role,
        permissions_json, client_version, last_sync_at
    ...
    return r({ error: "Failed to join alliance", details: err.message }, { status: 500 });
  }
});
router.post("/api/v1/universe/seed", async (req, env2) => {
  try {
    const body = await req.json().catch(() => ({}));
    const universeId = cleanUniverseId(body.universeId || "s267-en");
    const stats = await seedUniverseFromOfficialAPI(universeId, env2);
    return r({ success: true, universeId, stats });
  } catch (err) {
    console.error("Error in universe seeding:", err);
    return r({ error: "Failed to seed universe", details: err.message }, { status: 500 });
  }
});
router.get("/api/v1/universe/:universeId/status", async (req, env2) => {
  try {
    const url = new URL(req.url);
    const pathParts = url.pathname.split("/");
    const rawUniverseId = pathParts[pathParts.length - 2] || "s267-en";
    const universeId = cleanUniverseId(rawUniverseId);
    let planetCount = await env2.DB.prepare(
      `SELECT COUNT(*) as count FROM universe_slots WHERE universe_id = ?`
    ).bind(universeId).first();
    if (!planetCount || planetCount.count === 0) {
      try {
        ...
        await seedUniverseFromOfficialAPI(universeId, env2);
        planetCount = await env2.DB.prepare(
          `SELECT COUNT(*) as count FROM universe_slots WHERE universe_id = ?`
        ).bind(universeId).first();
      } catch (seedErr) {
    ...
    const moonCount = await env2.DB.prepare(
      `SELECT COUNT(*) as count FROM universe_slots WHERE universe_id = ? AND has_moon = 1`
    ).bind(universeId).first();
    const eventCount = await env2.DB.prepare(
    ...
    return r({
      ...
      serverName: info3?.server_name || "Universe",
      galaxies: info3?.galaxies || 9,
      systems: info3?.systems || 499,
      speed: info3?.speed || 1,
      speedFleet: info3?.speed_fleet || 1,
      debrisFactor: info3?.debris_factor || 0.3
    });
  } catch (err) {
    return r({ error: "Failed to get universe status", details: err.message }, { status: 500 });
  }
});
...
router.all("*", () => r({ error: "Endpoint not found" }, { status: 404 }));
var src_default = {
  fetch: (request, env2, ctx) => router.fetch(request, env2, ctx).catch((err) => {
    console.error("Unhandled edge error:", err);
    return r({ error: "Internal edge server error", details: err.message }, { status: 500 });
  })
};
...
// .wrangler/tmp/bundle-KHucEP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KHucEP/middleware-loader.entry.ts
