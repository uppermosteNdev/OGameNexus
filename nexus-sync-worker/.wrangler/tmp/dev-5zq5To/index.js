...
// .wrangler/tmp/bundle-QnvNPW/strip-cf-connecting-ip-header.js
...
async function seedUniverseFromOfficialAPI(rawUniverseId, env2) {
  ...
  for (const p2 of parsedPlanets) {
    ...
    if (!isInitialSeed && oldSlot) {
      ...
      const oldStatusNorm = (oldSlot.player_status || "active").split(",").map((s2) => s2.trim().toLowerCase()).sort().join(",");
      const newStatusNorm = (playerStatus || "active").split(",").map((s2) => s2.trim().toLowerCase()).sort().join(",");
      if (oldSlot.player_id && p2.playerId && oldStatusNorm !== newStatusNorm) {
        ...
        detectedEvents.push({
          ...
          playerId: p2.playerId,
          playerName,
          oldState: { playerStatus: oldSlot.player_status || "active" },
          newState: { playerStatus: playerStatus || "active" }
        });
      }
  ...
  try {
    ...
    for (const newSlot of slots) {
      ...
      if (oldSlot) {
        ...
        const oldStatusNorm = (oldSlot.player_status || "active").split(",").map((s2) => s2.trim().toLowerCase()).sort().join(",");
        const newStatusNorm = (newSlot.playerStatus || "active").split(",").map((s2) => s2.trim().toLowerCase()).sort().join(",");
        if (oldSlot.player_id && newSlot.playerId && oldStatusNorm !== newStatusNorm) {
          ...
          detectedEvents.push({
            ...
            playerName: newSlot.playerName || null,
            oldState: { playerStatus: oldSlot.player_status || "active" },
            newState: { playerStatus: newSlot.playerStatus || "active" },
            detectedAt: now
          });
        }
...
async function handleGetGalaxyEvents(req, env2) {
  try {
    ...
    const url = new URL(req.url);
    const universeId = cleanUniverseId(url.searchParams.get("universeId") || member.universe_id);
...
// .wrangler/tmp/bundle-QnvNPW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QnvNPW/middleware-loader.entry.ts
